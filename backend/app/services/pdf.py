...
class PDFService:
    ...
    async def generate_banenindeling_pdf(
        ...
        result = await self.db.execute(
            ...
            .options(
                joinedload(Speelronde.competitie),
                joinedload(Speelronde.baantoewijzingen).joinedload(BaanToewijzing.baan),
        ...
        sorted_toewijzingen = sorted(
            ronde.baantoewijzingen, key=lambda x: x.baan.nummer if x.baan else 0
        )
        ...
        for t in sorted_toewijzingen:
            ...
            data.append([str(t.baan.nummer) if t.baan else "-", team_info, tijd])
        ...
        footer_data = [
            [
                [
                    Paragraph(
                        "<b>Live stand & wijzigingen</b>",
                        ParagraphStyle("F1", fontSize=14, spaceAfter=5),
                    ),
                    Paragraph(
                        "Scan de QR-code voor de meest actuele banenindeling, uitslagen en live updates.",
                        styles["Normal"],
                    ),
                    Spacer(1, 5 * mm),
        ...
        elements.append(
            Paragraph(
                f"<i>Gegenereerd door Competitie-Planner.nl op {datetime.now().strftime('%d-%m-%Y %H:%M')}</i>",
                ParagraphStyle("Gen", fontSize=8, alignment=1, textColor=colors.grey),
            )
        )
    ...
    async def generate_seizoensoverzicht_pdf(
        ...
        result = await self.db.execute(select(Competitie).where(Competitie.id == competitie_id))
